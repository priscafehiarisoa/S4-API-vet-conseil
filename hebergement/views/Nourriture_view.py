...
def add_new_type_food(request):
    ...
    if(description != None):
        new_food = Nourriture(designation=designation, description=description, prix_par_unite=prix_par_unite, unite=unite)
        new_food.save()
        ...
        if request.method == 'GET':
            ...
            for race_id in races_selectionnees:
                ...
                attribution = Attribution(nourriture=nourriture, race=race, debut_interval_poids=debut_interval_poids, fin_interval_poids=fin_interval_poids)
                attribution.save()
