...
class Ajouter_hebergement_form(forms.Form):
    ...
    animal=forms.ModelChoiceField(queryset=Patient.objects.all(),widget=forms.Select(attrs={'class': 'form-control'}))
    date_debut_hebergement=forms.DateField(widget=forms.TextInput(attrs={'class': 'form-control','type':'date'}),input_formats=['%Y-%m-%d'])
    date_fin_hebergement=forms.DateField(widget=forms.TextInput(attrs={'class': 'form-control','type':'date'}),input_formats=['%Y-%m-%d'])
    type_nourriture=forms.ModelChoiceField(queryset=Nourriture.objects.all(),widget=forms.Select(attrs={'class': 'form-control'}))
    quantite=forms.FloatField(min_value=0,widget=forms.TextInput(attrs={'class': 'form-control','placeholder':'ex : 2.5'}))
